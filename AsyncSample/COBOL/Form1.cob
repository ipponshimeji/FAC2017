       ...
       REPOSITORY.
           ...
           DELEGATE DEL-ACTION-TASK-STRING EXPANDS GDEL-ACTION USING CLASS-TASK-STRING
      ...
      *非同期処理終了後に行いたい処理の本体
       ...
       LINKAGE SECTION.
       01 PARAM-TASK OBJECT REFERENCE CLASS-TASK-STRING.
      ...
      *「非同期処理終了後に行いたい処理本体」をGUIスレッド上で実行するメソッド
       METHOD-ID. CONTINUED-PROC PRIVATE.
       DATA DIVISION.
       WORKING-STORAGE SECTION.
       01 WK-TASK OBJECT REFERENCE CLASS-TASK-STRING.
       01 WK-PROC OBJECT REFERENCE DEL-ACTION-TASK-STRING.
       LINKAGE SECTION.
       01 PARAM-TASK OBJECT REFERENCE CLASS-TASK.
       PROCEDURE DIVISION USING BY VALUE PARAM-TASK.
      *    GUIスレッド上で処理を行う。
           SET WK-TASK TO PARAM-TASK AS CLASS-TASK-STRING.
           INVOKE DEL-ACTION-TASK-STRING "NEW" USING SELF N"CONTINUED-PROC-BODY" RETURNING WK-PROC.
           INVOKE SELF "Invoke" USING WK-PROC WK-TASK.
